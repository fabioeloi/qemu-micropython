#!/bin/bash
# Stress test the custom UART driver
set -e

PROJECT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SRC_DIR="$PROJECT_DIR/src"

echo "Running UART driver stress test..."

# Create a stress test program
cat > "$SRC_DIR/uart_stress_test.c" << EOF
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
...
# Compile the test
gcc -I"$SRC_DIR" -g -Wall -o "$PROJECT_DIR/build/uart_stress_test" \
    "$SRC_DIR/custom_uart_driver.c" "$SRC_DIR/uart_stress_test.c" -lm

# Run the test
"$PROJECT_DIR/build/uart_stress_test"

echo "UART stress tests completed" 